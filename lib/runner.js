...
const ArbitraryFiles = require('./arbitrary-files');
 ...
 * @typedef { import("./types/content").ArbitraryFile } ArbitraryFile
 ...
 * @param {ArbitraryFile[]} arbitraryFiles
...
function sendProjectContent(
  ...
  app.ports.collectArbitraryFiles.send(arbitraryFiles);
...
async function initializeApp(options, elmModulePath, reviewElmJson, appHash) {
  ...
  const requestedFilesP = new Promise((r) => {
    resolve = r;
  }).then((files) => ArbitraryFiles.collect(options, files));
  AppState.subscribe(app.ports.requestReadingFiles, resolve);

  requestedFilesP.then((files) => {
    console.log(files);
  });
  ...
  const [projectDeps, suppressedErrors, linksToRuleDocs, arbitraryFiles] =
    await Promise.all([
      projectDepsP,
      suppressedErrorsP,
      linksToRuleDocsP,
